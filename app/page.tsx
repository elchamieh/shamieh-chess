...
const levels = [
  { name: "Starters", text: "A welcoming first step into chess: the board, the pieces, the rules, and the habits that make learning enjoyable." },
  { name: "Beginners", text: "Build strong foundations through tactics, opening principles, calculation, and confident over-the-board play." },
  { name: "Intermediate", text: "Turn knowledge into consistent play with deeper calculation, strategy, endgames, and tournament discipline." },
  { name: "Advanced", text: "Prepare for serious competition with stronger analysis, positional understanding, preparation, and decision-making." },
];
...
export default async function HomePage() {
  ...
  const [{ data: tournaments }, { data: trainingSessions }] = await Promise.all([
    ...
    supabase
      ...
      .select("id, delivery_mode, session_date, start_time, end_time, class:classes(name, branch:branches(name), level:levels(name, sort_order))")
      .eq("active", true)
    ...
    <main className="public-site">
      ...
      <section className="public-hero">
        <div className="public-hero-copy">
          ...
          <p>A structured chess academy where players learn step by step, challenge themselves over the board, and grow into confident competitors — with classes in Saida and Beirut and tournaments open to the wider chess community.</p>
          <div className="public-actions">
            ...
            <a className="public-text-link" href="#schedule">Training dates ↓</a>
        ...
        <figure className="public-hero-media">
          <img src="/images/shamieh-tournament.webp" alt="Players competing in a Shamieh Chess tournament" width="1536" height="1024" fetchPriority="high" decoding="async" />
      ...
      <section className="public-section public-academy-story" id="academy">
        <div className="public-story-media">
          <img src="/images/shamieh-training.webp" alt="Shamieh Chess classroom training session" width="1536" height="1024" loading="lazy" decoding="async" />
        </div>
        <div className="public-story-copy">
          ...
          <p className="public-lead">Good chess development is more than learning moves. Students need the right ideas at the right time, regular practice, feedback, and opportunities to compete.</p>
          <p>At Shamieh Chess Academy, players progress through clear training levels so each student can build strong fundamentals, improve decision-making, and keep moving forward at an appropriate pace.</p>
        </div>
      </section>
        ...
        <div className="public-level-grid">
          {levels.map((level, index) => (
            <article className="public-level-card" key={level.name}>
              <span>0{index + 1}</span><h3>{level.name}</h3><p>{level.text}</p>
            </article>
          ))}
        </div>
      </section>
      ...
      <section className="public-community-band">
        <div className="public-community-photo">
          <img src="/images/shamieh-community.webp" alt="Shamieh Chess Academy students and coaches together" width="1440" height="1080" loading="lazy" decoding="async" />
        </div>
        <div className="public-community-copy">
          ...
          <p>Chess becomes more meaningful when players share the journey. Students train together, play over the board, take part in events, and become part of an active chess community that celebrates effort as much as results.</p>
        </div>
      </section>
      ...
      <section className="public-section public-locations" id="locations">
        ...
        <div className="public-location-grid">
          <article className="public-location-card">
            <div className="location-marker">01</div><div><h3>Saida</h3><p>Structured academy classes for players progressing from their first chess lessons to advanced competitive training.</p><Link href="/register">Register for Saida →</Link></div>
          </article>
          <article className="public-location-card">
            <div className="location-marker">02</div><div><h3>Beirut</h3><p>Join the same Shamieh Chess training pathway in Beirut, with level-based coaching and opportunities to compete.</p><Link href="/register">Register for Beirut →</Link></div>
          </article>
        </div>
      ...
      <PublicTrainingSchedule rows={(trainingSessions || []) as any} />

      <section className="public-section public-tournaments" id="tournaments">
        <div className="public-tournament-intro">
          <div className="public-tournament-photo"><img src="/images/shamieh-achievements.webp" alt="Shamieh Chess tournament players with trophies and medals" width="1536" height="1024" loading="lazy" decoding="async" /></div>
          <div className="public-section-heading">
            <span className="public-eyebrow">FROM TRAINING TO COMPETITION</span>
            <h2>Play. Compete. Improve.</h2>
            <p>Competition is part of development. Shamieh Chess tournaments give academy students and other players a place to test their preparation, gain experience, and enjoy serious over-the-board chess.</p>
            <Link className="btn secondary" href="/tournaments">See all tournaments</Link>
          </div>
        </div>

        {!tournaments?.length ? <div className="public-empty">New tournaments will be announced here.</div> : (
            ...
            {tournaments.map((tournament: any) => {
              ...
              return (
                <article className="public-tournament-card" key={tournament.id}>
                  ...
                  <p className="public-muted">{[tournament.branch?.name, tournament.venue].filter(Boolean).join(" · ") || "Location to be announced"}</p>
                  <div className="public-tournament-meta"><span>{formatFee(tournament.fee_amount, tournament.fee_currency)}</span><span className={`pill ${canRegister ? "" : "public-pill-muted"}`}>{canRegister ? "Registration open" : "Registration closed"}</span></div>
      ...
      <section className="public-cta">
        <div><span className="public-eyebrow">YOUR NEXT MOVE</span><h2>Ready to make chess part of the week?</h2><p>Join the academy in Saida or Beirut, or register for an upcoming public tournament.</p></div>
        <div className="public-actions"><Link className="btn public-primary" href="/register">Join Shamieh Chess</Link><Link className="btn secondary" href="/tournaments">Tournament Registration</Link></div>
      </section>
